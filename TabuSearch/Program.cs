...
namespace TabuSearch
{
    class Program
    {
        ...
        static void runExperiment1(int n1, int n2, MyGraph graph) {  
            int experiment = 0;  
            int timeRestart = 0;  
            int maxTime1 = n1;  
            int maxTime2 = n2;  
            ...
            {  
                ...
                {  
                    ...
                    Console.WriteLine("**********************************************");  
                    Console.WriteLine("                   Graph " + (experiment + 1));  
                    Console.WriteLine("**********************************************");  
        
                    if (i == 1)  
                {  
                        stopwatch.Start();  
                        maxClique = FindMaxClique(graph, maxTime1, targetCliqueSize, timeRestart);  
                        stopwatch.Stop();  
                    }  
                    else  
                    {  
                        stopwatch.Start();  
                        maxClique = FindMaxClique(graph, maxTime2, targetCliqueSize, timeRestart);  
                        stopwatch.Stop();  
                    }  
        
                    Console.WriteLine("Size of best clique found : " + maxClique.Count);  
                    Console.WriteLine("\nBest clique found : " + ListAsString(maxClique));  
        
                    timeTaken = (double)(stopwatch.Elapsed.TotalMilliseconds / 1000);  
                    Console.WriteLine("Elapsed Time : " + timeTaken + " seconds\n");  
                ...
                experiment = 0;  
                Console.WriteLine("------------------------------------------------");  
                Console.WriteLine("  Average Times Taken  " + (totalTime / 30));  
                Console.WriteLine("------------------------------------------------\n");  
            }  
        
            Console.WriteLine("End tabu agorithm maximum clique demo\n");  
            Console.ReadLine();  
                    
        } 
        static void runExperiment2(int n1, int n2, MyGraph graph)  
        {  
            int experiment = 0;  
            int timeRestart1 = n1;  
            int timeRestart2 = n2;  
            ...
            {  
                ...
                {  
                    ...
                    {  
                        stopwatch.Start();  
                        maxClique = FindMaxClique(graph, maxTime, targetCliqueSize, timeRestart1);  
                        stopwatch.Stop();  
                    }  
                    else  
                    {  
                        stopwatch.Start();  
                        maxClique = FindMaxClique(graph, maxTime, targetCliqueSize, timeRestart2);  
                        stopwatch.Stop();  
                    }  
                    Console.WriteLine("Size of best clique found : " + maxClique.Count);  
                    Console.WriteLine("\nBest clique found : " + ListAsString(maxClique));  
        ...
        static void runExperiment3(int n, MyGraph graph)  
        {  
            int experiment = 0;  
            int timeRestart = 0;  
            int maxTime = 50;  
            int targetCliqueSize = n;  //CHANGE THIS TO N?
            double totalTime = 0;  
            ...
            {  
                ...
                stopwatch.Start();  
                List<int> maxClique = FindMaxClique(graph, maxTime, targetCliqueSize, timeRestart);  
                stopwatch.Stop();  
        ...
        static List<int> FindMaxClique(MyGraph graph, int maxTime, int targetCliqueSize, int timeRestart)  
        {  
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                // List<int> maxClique = FindMaxClique(graph, maxTime, targetCliqueSize, maxTime);
                ...
                //runExperiment1(100000, 1000, graph);
                //runExperiment2(1000, 100000, graph);
                runExperiment3(10, graph);
            }
            catch (Exception ex)
            {
